...
#https://crispr.cos.uni-heidelberg.de/result/fa46264f433515752e2a9272a8ee83848b00de37/unnamed.xls
    ...
    time.sleep(5)
    chrome.close()
    if os.path.exists(root+'web1files\\'+name+'.xls')!=True:
        option =  webdriver.ChromeOptions()
        prefs = {'profile.default_content_settings.popups': 0, 'download.default_directory': root+'web1files\\'}
        option.add_experimental_option('prefs', prefs)
        chrome = webdriver.Chrome(chrome_options=option)
    ...
    try:
        ...
        with open(root+'web1files\\'+'download_error.log','a+') as l:
            l.write(seq+'_'+species+'_'+name+'_'+str(i)+'\n')
            l.close()
        return 'Error1'
    except IndexError:
        with open(root+'web1files\\'+'multiseq_error.log','a+') as l:
            l.write(seq+'_'+species+'_'+name+'_'+str(i)+'\n')
            l.close() 
        return 'Error2'
    return 'OK'
